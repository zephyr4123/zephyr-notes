...
12. **concept 和 case 都要写过程，不是只列结果。** 读者画像：有大学数学基础（微积分、概率论）但不以数学为生。每一步推导都写出来，每一步都说清为什么这么做、不这么做会怎样；不许只列结果、不许一上来套公式算数。先讲清原理再给应用例子。case 同样：从问题到数据到方法到结果到复盘，每一步怎么做的、为什么这么做、数字怎么算出来的都写出来，读者要能照着复现。写完自问：一个没经历过这件事、没学过这个方法的人能不能顺着读懂并自己做一遍。
