...
related: [chi-square-test, hypergeometric-distribution, binomial-distribution, hypothesis-testing]
status: stable
created: 2026-09-08
updated: 2026-09-08
---
...
假设检验的套路是反证法：先假设两组**没有**差别（原假设 $H_0$），然后问，在这个假设下，出现当前这么大或更大的差距的概率有多大。这个概率就是 $p$ 值。整套框架（$H_0$、零分布、$p$ 值、两类错误、功效）单独写在[假设检验与 p 值](hypothesis-testing.md)里，这里只用到它的骨架。$p$ 很小，说明"没差别"这个假设下几乎看不到这样的数据，那就倾向于推翻它。
...
那么组 1 的成功数 $A$ 服从[二项分布](../probability/binomial-distribution.md) $\mathrm{Bin}(r_1,\pi_1)$，组 2 的成功数 $C$ 服从 $\mathrm{Bin}(r_2,\pi_2)$，两者独立。原假设：
...
约掉之后剩下的式子里没有 $\pi$。这就是 Fisher 精确检验的全部核心：**条件在总成功数上，未知参数消失，分布完全已知。** 剩下的分布叫[超几何分布](../probability/hypergeometric-distribution.md)，它的期望、方差和与二项分布的关系在那篇里单独推。
