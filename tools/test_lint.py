...
## 为什么用它
见 [Fisher 精确检验](../concept/fisher-exact-test.md)。
...
class VaultCase(unittest.TestCase):
    def setUp(self):
        ...
        for d in ("notes/concept", "notes/case", "notes/problem", "notes/qa", "notes/pattern", "maps", "assets", "inbox"):
...
class TestFrontmatter(VaultCase):
    def test_missing_frontmatter(self):
        self.valid_vault()
        self.write("notes/concept/x.md", "# 没有 frontmatter\n")
        self.assert_error(self.run_lint(), "缺少 frontmatter")

    def test_unclosed_frontmatter(self):
        self.valid_vault()
        self.write("notes/concept/x.md", "---\nid: x\n")
        self.assert_error(self.run_lint(), "没有闭合")

    def test_empty_summary(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("summary: 2×2 列联表小样本下的精确 p 值。", 'summary: ""'))
        self.assert_error(self.run_lint(), "必填字段 `summary` 为空")

    def test_undeclared_field(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("status: stable", "status: stable\nfoo: bar"))
        self.assert_error(self.run_lint(), "未声明的字段 `foo`")

    def test_missing_required_field(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("status: stable\n", ""))
        self.assert_error(self.run_lint(), "缺少必填字段 `status`")

    def test_bad_enum(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("status: stable", "status: done"))
        self.assert_error(self.run_lint(), "取值 `done` 不在")

    def test_updated_before_created(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("updated: 2026-09-07", "updated: 2026-08-01"))
        self.assert_error(self.run_lint(), "早于 created")

    def test_bad_date(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("created: 2026-09-01", "created: 2026/09/01"))
        self.assert_error(self.run_lint(), "不是合法日期")

    def test_block_list_syntax_accepted(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("tags: [statistics, hypothesis-testing]", "tags:\n  - statistics\n  - hypothesis-testing"))
        self.assertTrue(self.run_lint().ok)

    def test_optional_string_may_be_empty(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("aliases: [费舍尔精确检验]", "aliases: []"))
        self.assertTrue(self.run_lint().ok)
...
class TestIds(VaultCase):
    def test_id_mismatch_filename(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("id: fisher-exact-test", "id: fisher"))
        self.assert_error(self.run_lint(), "与文件名")

    def test_bad_id_format(self):
        self.valid_vault()
        self.write("notes/concept/Fisher_Test.md", CONCEPT.replace("id: fisher-exact-test", "id: Fisher_Test"))
...
class TestTags(VaultCase):
    def test_unknown_tag(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("tags: [statistics, hypothesis-testing]", "tags: [machine-learning]"))
    ...
    def test_duplicate_tags(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("tags: [statistics, hypothesis-testing]", "tags: [statistics, statistics]"))
...
class TestEdges(VaultCase):
    ...
    def test_self_reference(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("related: []", "related: [fisher-exact-test]"))
        self.assert_error(self.run_lint(), "引用了自己")

    def test_ref_written_as_path_rejected(self):
        self.valid_vault()
        self.write("notes/case/ab-test-small-sample-fisher.md", CASE.replace("uses: [fisher-exact-test]", "uses: [../concept/fisher-exact-test.md]"))
...
class TestBody(VaultCase):
    def test_broken_link(self):
        self.valid_vault()
        self.write("notes/case/ab-test-small-sample-fisher.md", CASE.replace("../concept/fisher-exact-test.md", "../concept/nope.md"))
        self.assert_error(self.run_lint(), "链接目标不存在")

    def test_wikilink_rejected(self):
        self.valid_vault()
        self.write("notes/case/ab-test-small-sample-fisher.md", CASE.replace("[Fisher 精确检验](../concept/fisher-exact-test.md)", "[[fisher-exact-test]]"))
    ...
    def test_missing_required_heading(self):
        self.valid_vault()
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("## 适用边界\n", "## 边界\n"))
    ...
    def test_image_outside_assets(self):
        self.valid_vault()
        self.write("notes/concept/pic.png", "x")  # 也会触发「只放 .md」
        self.write("notes/concept/fisher-exact-test.md", CONCEPT + "\n![p](pic.png)\n")
...
class TestReachability(VaultCase):
    def test_orphan_note(self):
        self.valid_vault()
        self.write("notes/concept/lonely.md", CONCEPT.replace("id: fisher-exact-test", "id: lonely"))
        self.assert_error(self.run_lint(), "孤儿")

    def test_note_reachable_only_via_related(self):
        self.valid_vault()
        self.write("notes/concept/chi-square-test.md", CONCEPT.replace("id: fisher-exact-test", "id: chi-square-test"))
        self.write("notes/concept/fisher-exact-test.md", CONCEPT.replace("related: []", "related: [chi-square-test]"))
        self.assertTrue(self.run_lint().ok)
...
class TestStructure(VaultCase):
    ...
    def test_subdir_in_type_dir(self):
        self.valid_vault()
        (self.tmp / "notes/concept/sub").mkdir()
...
class TestNew(VaultCase):
    ...
    def test_new_without_summary_fails_lint(self):
        self.write("maps/home.md", MAP.replace("id: math-in-biz", "id: home").replace("- [小样本 AB 用 Fisher 判显著](../notes/case/ab-test-small-sample-fisher.md) ← 先看这个", "<!-- 空 -->"))
        new.create(self.tmp, "concept", "x", "X", tags=["statistics"], map_id="home", today=TODAY)
        self.assert_error(self.run_lint(), "必填字段 `summary` 为空")

    def test_new_attaches_to_map_unsorted_section(self):
        new.create(self.tmp, "map", "home", "首页", today=TODAY)
        new.create(self.tmp, "concept", "x", "X 原理", tags=["statistics"], map_id="home", today=TODAY)
        text = (self.tmp / "maps/home.md").read_text(encoding="utf-8")
        self.assertIn("## 未归类\n- [X 原理](../notes/concept/x.md)", text)
    ...
    def test_new_attach_appends_within_unsorted_before_next_heading(self):
        ...
        new.create(self.tmp, "concept", "a", "A", tags=["statistics"], map_id="home", today=TODAY)
        new.create(self.tmp, "concept", "b", "B", tags=["statistics"], map_id="home", today=TODAY)
        text = home.read_text(encoding="utf-8")
        self.assertIn("## 未归类\n- [A](../notes/concept/a.md)\n- [B](../notes/concept/b.md)\n\n## 其他", text)

    def test_new_rejects_duplicate_id(self):
        new.create(self.tmp, "concept", "x", "X", today=TODAY)
